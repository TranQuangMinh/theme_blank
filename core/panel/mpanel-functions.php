...
function tie_admin_register() {
    ...
    wp_register_script( 'tie-admin-slider', CORE_PATH . '/panel/js/jquery.ui.slider.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-widget', 'jquery-ui-mouse', 'jquery-ui-sortable' ) , false , false );  
    wp_register_script( 'tie-admin-main', CORE_PATH . '/panel/js/tie.js', array( 'jquery' ) , false , false );  

	wp_register_style( 'tie-style', CORE_PATH .'/panel/style.css', array(), '20120208', 'all' ); 
...
function tie_add_admin() {
	...
	$icon = CORE_PATH . '/panel/images/general.png';
	...
	switch ( $value['type'] ) {
		...
		?>
				...
				<div id="<?php echo $value['id']; ?>-preview" class="img-preview" <?php if(!tie_get_option( $value['id'] )) echo 'style="display:none;"' ?>>
					<img src="<?php if(tie_get_option( $value['id'] )) echo tie_get_option( $value['id'] ); else echo CORE_PATH . '/panel/images/spacer.png'; ?>" alt="" />
